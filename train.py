...
def train(run,
    ...
    for batch in tqdm(train_dataloader):
        ...
        if i % log_loss_every == 0 and i != 0:
            run.log({"Train Gen loss" : generator_loss}, step=epoch * len(train_dataloader) + i)
        ...
        if i % log_audio_every == 0 and i != 0:
            reconstructed_wav = pred_wave[0].detach().cpu().numpy()
            run.log({"Train Predicted Audio" : wandb.Audio(reconstructed_wav, 22050)}, step=epoch * len(train_dataloader) + i)
            reconstructed_wav = real_wave[0].detach().cpu().numpy()
            run.log({"Train Real Audio" : wandb.Audio(reconstructed_wav, 22050)}, step=epoch * len(train_dataloader) + i)
        i += 1
...
if __name__ == '__main__':
    ...
    train_dataloader = DataLoader(LJSpeechDataset('.'), batch_size=1, collate_fn=collator)
